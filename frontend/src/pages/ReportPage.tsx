    ...
    <div className="mx-auto max-w-3xl space-y-5">
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-mute">{r.report_type}</p>
        </div>
        <CopyButton
      ...
      <Card title="Header">
        <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm sm:grid-cols-3">
          ...
          <Detail label="Reported address">
            <Mono value={r.header.reported_address} />
          </Detail>
          <Detail label="Status">
            {r.header.status}
      ...
      <Card title={`VASP candidates (${r.vasp_candidates.length})`}>
        {r.vasp_candidates.length === 0 ? (
          <p className="text-sm text-mute">No VASP attributed.</p>
        ) : (
          <ul className="space-y-3">
            {r.vasp_candidates.map((c) => (
              <li
                key={c.rank}
                className="rounded-sm border border-navy-700 p-3"
              >
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-sm font-semibold text-slate-100">
                    #{c.rank} {c.name ?? c.label ?? "Unattributed"}
                  </span>
                  <span className="text-xs uppercase tracking-wide text-mute">
                    {c.tier} · {c.verified ? "verified" : "heuristic"}
                  </span>
                ...
                {c.confidence_formula && (
                  <details className="mt-2 text-xs">
                    <summary className="cursor-pointer text-mute hover:text-slate-300">
                      Confidence arithmetic (raw {c.confidence_formula.raw_score}{" "}
                      → {c.confidence_formula.score})
                    </summary>
                      ...
                      <tbody className="font-mono">
                        {Object.keys(c.confidence_formula.terms).map((k) => (
                          <tr key={k} className="border-t border-navy-700">
                            <td className="py-1 pr-3 font-sans text-mute">
                              {k}
                            </td>
      ...
      <Card title={`Fund flow (${r.fund_flow.length} steps)`}>
        {r.fund_flow.length === 0 ? (
          <p className="text-sm text-mute">No transfers recorded.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {r.fund_flow.map((s) => (
                  <tr key={s.step} className="border-b border-navy-800">
      ...
      <Card title="Data sources">
        {r.data_sources.length === 0 ? (
          <p className="text-sm text-mute">No snapshots recorded.</p>
        ) : (
          <ul className="space-y-2 text-xs">
            {r.data_sources.map((s, i) => (
              <li key={i} className="border-b border-navy-800 pb-2">
                ...
                <div className="font-mono text-mute break-all">
      ...
      <Card title="Certification">
        <p className="text-sm text-slate-300">{r.certification.statement}</p>
        <dl className="mt-2 text-xs text-mute">
